...
function News({ tweets, logged, users }: HomeProps) {
  ...
  const handleTrendingSort = async (sort: string) => {
    ...
    try {
      const res = await api.get(`news?per_page=25&page=0&sort_by=${sort}`, {
        withCredentials: true,
      });
...
export const getServerSideProps: GetServerSideProps<HomeProps> = async (
  ...
  const res = await api.get('news?per_page=25&page=0', {
    withCredentials: true,
  ...
  const res2 = await api.get('news/top-users', {
    withCredentials: true,
    headers: { cookie: ctx.req?.headers?.cookie ?? null },
  });
